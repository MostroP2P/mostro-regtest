...
lncli() {
  ...
  docker_compose exec --user lnd $lnd_service_name lncli --network=regtest --rpcserver=localhost:10001 "$@"
}
...
lightning_open_channels() {
  lnd_open_channel mostro_lnd alice_lnd $(lnd_node_pubkey alice_lnd)
  lnd_open_channel alice_lnd bob_lnd $(lnd_node_pubkey bob_lnd)
  lnd_open_channel bob_lnd mostro_lnd $(lnd_node_pubkey mostro_lnd)
